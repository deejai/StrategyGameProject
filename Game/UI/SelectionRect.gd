...
func set_end_point(point: Vector2):
	end_point = point
	var topleft = Vector2(min(start_point.x, end_point.x), min(start_point.y, end_point.y))
	var botright = Vector2(max(start_point.x, end_point.x), max(start_point.y, end_point.y))
